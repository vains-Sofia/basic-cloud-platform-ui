				...
				<el-collapse class="example-collapse">
					<el-collapse-item title="ISO 8601 格式说明" name="timer">
						<div class="timer-examples">
							...
							<div class="example-item">
								<strong>持续时间：</strong>
								<code>PT5M</code> (5分钟), <code>PT1H</code> (1小时),
								<code>P1D</code> (1天)
...
import { getBusinessObject, getEventDefinition, hasEventDefinition } from '../utils/bpmnHelper'
...
const updateTimerDefinition = () => {
	...
	updateProperties({ eventDefinitions: [timerDef] })
}

const updateMessageDefinition = () => {
	const messageDef = getOrCreate(
		'bpmn:MessageEventDefinition',
		formData.value.messageRef ? `Message_${formData.value.messageRef}` : undefined,
	)
	if (formData.value.messageRef || formData.value.messageName) {
		messageDef.messageRef = getOrCreate(
			'bpmn:Message',
			formData.value.messageRef,
			formData.value.messageName,
		)
	}

	updateProperties({ eventDefinitions: [messageDef] })
	// const messageDef = moddle.value.create('bpmn:MessageEventDefinition')
	// if (formData.value.messageRef || formData.value.messageName) {
	// 	messageDef.messageRef = moddle.value.create('bpmn:Message', {
...
const updateErrorDefinition = () => {
	const errorDef = getOrCreate(
		'bpmn:ErrorEventDefinition',
		formData.value.errorCode ? `Definition_${formData.value.errorCode}` : undefined,
	)
	if (formData.value.errorCode || formData.value.errorMessage) {
		errorDef.errorRef = getOrCreate(
			'bpmn:Error',
			`Error_${formData.value.errorCode}`,
			formData.value.errorMessage,
			{
				id: `Error_${formData.value.errorCode}`,
	...
	updateProperties({ eventDefinitions: [errorDef] })

	/*	const errorDef = moddle.value.create('bpmn:ErrorEventDefinition')
	if (formData.value.errorCode || formData.value.errorMessage) {
		errorDef.errorRef = moddle.value.create('bpmn:Error', {
			id: `Error_${Date.now()}`,
			errorCode: formData.value.errorCode,
			errorMessage: formData.value.errorMessage,
		})
...
const updateSignalDefinition = () => {
	const signalDef = getOrCreate(
		'bpmn:SignalEventDefinition',
		formData.value.signalRef ? `Definition_${formData.value.signalRef}` : undefined,
	)
	if (formData.value.signalRef || formData.value.signalName) {
		signalDef.signalRef = getOrCreate(
			'bpmn:Signal',
			formData.value.signalRef,
			formData.value.signalName,
		)
	}
	signalDef.scope = formData.value.signalScope
	updateProperties({ eventDefinitions: [signalDef] })

	/*const signalDef = moddle.value.create('bpmn:SignalEventDefinition')
	if (formData.value.signalRef || formData.value.signalName) {
		signalDef.signalRef = moddle.value.create('bpmn:Signal', {
...
const updateEscalationDefinition = () => {
	const escDef = getOrCreate(
		'bpmn:EscalationEventDefinition',
		formData.value.escalationCode ? `Escalation_${formData.value.escalationCode}` : undefined,
	)
	if (formData.value.escalationCode || formData.value.escalationName) {
		escDef.escalationRef = getOrCreate(
			'bpmn:Escalation',
			`Escalation_${formData.value.escalationCode}`,
			formData.value.escalationName,
			{
				id: `Escalation_${formData.value.escalationCode}`,
	...
	updateProperties({ eventDefinitions: [escDef] })
	// const escDef = moddle.value.create('bpmn:EscalationEventDefinition')
	// if (formData.value.escalationCode || formData.value.escalationName) {
	// 	escDef.escalationRef = moddle.value.create('bpmn:Escalation', {
...
const updateConditionalDefinition = () => {
	...
	updateProperties({ eventDefinitions: [condDef] })
}
...
const updateLinkDefinition = () => {
	...
	updateProperties({ eventDefinitions: [linkDef] })
}
...
function getOrCreate(type: string, id?: string, name?: string, data?: any) {
	let eventElement = null

	if (id) {
		eventElement = findById(id, type)
	}

	if (!eventElement && name) {
		eventElement = findByName(name, type)
	}

	// 不存在，则创建
	if (!eventElement) {
		eventElement = createElement(type, id, name, data)
	} else {
		// 存在 → 修改
		if (data) {
			Object.assign(eventElement, data)
		} else {
			if (id) {
				eventElement.id = id
			}
...
const getAllByType = (type: string) => {
	const root = props.modeler.get('canvas').getRootElement()
	const definitions = root.businessObject.$parent
	return definitions.rootElements.filter((e: any) => e.$type === type)
}

function findById(id: string, type: string) {
	return getAllByType(type)?.find((e: any) => e.id === id)
}

function findByName(name: string, type: string) {
	return getAllByType(type)?.find((e: any) => e.name === name)
}

function createElement(type: string, id?: string, name?: string, data?: any) {
	...
	if (data) {
		...
		initData = {
			id: id || `${type.replace('bpmn:', '')}_${Math.random().toString(36).slice(2, 10)}`,
			name,
		}
	}
	if (!initData.id) {
		initData.id = `${type.replace('bpmn:', '')}_${Math.random().toString(36).slice(2, 10)}`
	}
	const signal = moddle.create(type, initData)
