...
import type { PageDeployDefinitionResponse } from '@/api/types/DeploymentDefinitionTypes.ts'
...
export function useDeploymentDefinition() {
	...
	// 表格数据列表
	const dataList = ref<PageDeployDefinitionResponse[]>([])
	// 表格分页
	 ...
	 * @param active 激活/挂起
	 */
	const changeSuspension = (row: PageDeployDefinitionResponse, active: boolean) => {
		let changeState
		if (active) {
			changeState = { state: 1 }
		} else {
	 ...
	 * @param row 流程定义
	 */
	const viewBpmnXml = (row: PageDeployDefinitionResponse) => {
	 ...
	 * @param row 流程定义数据
	 */
	const toProcessDetails = (row: PageDeployDefinitionResponse) => {
